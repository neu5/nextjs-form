...
export default function GroupMember({
  ...
  const shouldShowTransportInfo = !IS_TRANSPORTATION_ENABLED && mode === 'EDIT';
  ...
  return (
    <div className="mb-8">
      ...
      {isInstitution ? (
        ...
        <div className="mb-4 mt-4">
          ...
          <div className="relative mt-2 rounded-md">
            ...
            <div
              id={`group-guardian-name-${id}-error`}
              aria-live="polite"
              aria-atomic="true"
            >
              {memberErrors?.guardianName &&
                memberErrors.guardianName.map((error: string) => (
                  <p className="mt-2 text-sm text-red-500" key={error}>
                    {error}
                  </p>
                ))}
      ...
      {/* Transport */}
      {(IS_TRANSPORTATION_ENABLED || shouldShowTransportInfo) &&
        transports.length > 0 && (
          <>
            <div className="mb-4 mt-4">
              <label
                ...
                <div className="relative">
                  <select
                    ...
                    className={clsx(
                      'peer block w-full cursor-pointer rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500',
                      {
                        'opacity-50': shouldShowTransportInfo,
                      },
                    )}
                    aria-describedby="transport-id-error"
                    value={transportId}
                    disabled={shouldShowTransportInfo}
                    onChange={(ev) =>
                      saveMember({
                        id,
                        name: 'transportId',
